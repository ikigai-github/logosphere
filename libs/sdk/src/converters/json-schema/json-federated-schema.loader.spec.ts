...
describe('Federated Schema Loader', () => {
  test('Load federated JSON schema', () => {
    ...
    expect(
      federatedSchema.find(
        (schema: JsonFederatedSchema) => schema.module === 'user'
      )
    ).toBeDefined();
    expect(
      federatedSchema.find(
        (schema: JsonFederatedSchema) => schema.module === 'minting'
      )
    ).toBeDefined();
    expect(
      federatedSchema.find(
        (schema: JsonFederatedSchema) => schema.module === 'auction'
      )
    ).toBeDefined();
  });
});
